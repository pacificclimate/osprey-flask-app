...
def handle_run(arg):
    ...
    if valid:
        ...
        print(url)
        input_response = requests.get(f'{base_url}/osprey/input?case_id=sample&{url}').content
...
control_box = Box(children = [start_date, end_date, latitude, longitude, model, add_point, point_list, remove_point, run], layout=box_layout)
